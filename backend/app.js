...
// Security middleware
app.use(helmet());
// Allow the frontend origin to be set via FRONTEND_URL or VITE_FRONTEND_URL.
// When credentials: true is used, browsers disallow a wildcard '*' origin,
// so we reflect the configured origin (or the request origin) instead.
// For development with multiple frontend instances, use true to reflect any localhost origin.
const FRONTEND_URL = true; // Always reflect the request origin for flexibility in dev
app.use(cors({
  origin: FRONTEND_URL,
